...
//------パブリックメソッドs--------
insertDocument = async function (colName, insertObj) {
...
insertManyDocuments = async function (colName, insertObj) {
  const database = client.db(dbName);
  const collection = database.collection(colName);
...
findManyDocuments = async function (colName, queryObj, outputFieldObj) {
  const database = client.db(dbName);
  const collection = database.collection(colName);
...
updateDocument = async function (colName, queryObj, updateObj) {
  const database = client.db(dbName);
  const collection = database.collection(colName);
...
deleteManyDocuments = async function (colName, queryObj) {
  const database = client.db(dbName);
  const collection = database.collection(colName);
